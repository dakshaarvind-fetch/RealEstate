...
def _store_pending(pending: _PendingPayment) -> None:
    _cleanup_expired()
    # Cancel any previous pending payment for this user
...
async def _create_sheet_for_pending(pending: _PendingPayment) -> str:
    ...
    Sheet is always created under GOOGLE_SHEET_USER_ID if set (single-tenant mode),
    otherwise falls back to the requester's user_id.
    """
...
async def on_commit(ctx: Context, sender: str, msg: CommitPayment):
    ...
    # Secondary lookup: transaction_id might be a PaymentIntent ID (e.g. from test script).
    # Scan pending payments by the sender's agent address (stored in pending.sender).
    ...
    # Verify with Stripe (accepts both checkout session IDs and PaymentIntent IDs)
    ctx.logger.info(f"[on_commit] Verifying payment: {transaction_id}")
    try:
    ...
    ctx.logger.info(f"[on_commit] verify_payment={paid}")
    if not paid:
        # Agentverse sends CommitPayment as soon as the user clicks "Confirm", sometimes
        # before Stripe has finished processing. Retry a few times before rejecting.
        for delay in (2, 4, 6):
            ...
            try:
                ...
                break
            ctx.logger.info(f"[on_commit] retry verify_payment={paid} (after {delay}s)")
            if paid:
                break
    ...
    # Payment confirmed — create the sheet
    ctx.logger.info("[on_commit] Payment verified — creating sheet...")
    try:
        sheet_url = await _create_sheet_for_pending(pending)
    except GoogleAuthRequiredError as exc:
        ctx.logger.error(f"[on_commit] Google auth required for sheet_user_id — {exc}")
    ...
    ctx.logger.info(f"[on_commit] Sheet created: {sheet_url}")

    # Clean up pending state
    _pending_payments.pop(pending.checkout_session_id, None)
    _pending_by_user.pop(pending.user_id, None)

    # Acknowledge payment completion to the wallet
    try:
        await ctx.send(sender, CompletePayment(transaction_id=pending.checkout_session_id))
    except Exception as exc:
        ctx.logger.warning(f"[on_commit] CompletePayment send failed: {exc}")

    # Deliver the sheet to the original requester
    num_results = len(pending.df) if pending.df is not None else 0
    ctx.logger.info(f"[on_commit] Delivering sheet to {pending.sender} (is_chat={pending.is_chat})")
    try:
        if pending.is_chat:
            await ctx.send(
                pending.sender,
        ...
        ctx.logger.info(f"[on_commit] Sheet delivered to {pending.sender}: {sheet_url}")
    except Exception as exc:
        ctx.logger.error(f"[on_commit] Sheet delivery FAILED: {exc} — URL was: {sheet_url}")
...
async def _search_and_request_payment(
    ...
    Run search-only workflow, then:
      • If Stripe configured and listings found → create Stripe checkout, send
        RequestPayment (+ summary message). The sheet is created on CommitPayment.
    ...
    if result.num_results == 0 or result.pending_df is None:
        # No results — no payment needed, just reply
        if is_chat:
            await ctx.send(
                sender,
    ...
    # Send payment request via payment protocol
    await ctx.send(
        sender,
        RequestPayment(
    ...
    # Also send a human-readable summary so non-payment-aware clients see context
    summary_text = (
        f"{result.summary}\n\n"
...
async def handle_search(ctx: Context, sender: str, msg: SearchRequest):
    ...
    # Google-auth shortcut — no payment needed
...
async def handle_followup(ctx: Context, sender: str, msg: FollowUpRequest):
    ...
    if not stripe_configured():
        # Free mode — use existing resume logic
        try:
    ...
    # Stripe mode — follow-ups also require payment for a new sheet
    try:
        await _search_and_request_payment(
            ctx, sender, msg.query, user_id=user_id, is_chat=False
        )
